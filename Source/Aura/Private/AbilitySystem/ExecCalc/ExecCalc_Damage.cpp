...
struct AuraDamageStatics
{
	...
	//在这里区分获取到的属性值是自己的Source还是目标的Target
	AuraDamageStatics()
	{
	...
	const TMap<FGameplayTag, FGameplayEffectAttributeCaptureDefinition>& InTagsToDefs) const
{
	...
	for (TTuple<FGameplayTag, FGameplayTag> Pair : GameplayTags.DamageTypesToDebuffs)
	{
		...
		//返回SetByCaller修饰符的大小。将返回0。f和警告，如果震级尚未设置。
		const float TypeDamage = Spec.GetSetByCallerMagnitude(DamageType, false, -1.f);

		if (TypeDamage > -.5f)
		{
			//Determine if there was a sucessfull debuff
                                              ...
                                              FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	//获取Sources 和 Target的AbilitySystemcomponent
	...
	//通过角色Actor是否绑定CombatInterface接口，来获取Source和Target角色的等级
	...
	*/
	...
	for (const TTuple<FGameplayTag, FGameplayTag>& Pair : FAuraGameplayTags::Get().DamageTypesToResistances)
	{
		...
		checkf(TagsToCaptureDefs.Contains(ResistanceTag), TEXT("Doesn't contains tag : %s"), *ResistanceTag.ToString());
		...
		if (UAuraAbilitySystemLibrary::IsRaidalDamage(EffectContextHandle))
		{
			...
			// 1. 在AuraCharacterBase中重写TakeDamage。
			// 2. 创建委托OnDamageDelegate，广播在TakeDamage中收到的损害
			// 3. 将lambda绑定到受害者的OnDamageDelegate。
			// 4. 调用UGameplayStatics::ApplyRadialDamageWithFalloff来造成伤害（这将导致TakeDamage在受害者上被调用，然后将广播OnDamageDelegate）
			// 5. 在Lambda中，设置DamageTypeValue为从广播接收到的损害值
